(() => {
  ...
  const passwordResetUiText = () => ({
    en: {
      forgot: "Forgot password?",
      requestTitle: "Reset password",
      requestText: "Enter the email used for your KisaMore account. We will send a password reset link.",
      requestSubmit: "Send reset link",
      requestSuccess: "If this email belongs to an account, a reset link has been sent.",
      confirmTitle: "Set a new password",
      confirmText: "Enter a new password for your KisaMore account.",
      confirmSubmit: "Save new password",
      confirmSuccess: "Password changed. You can now sign in.",
      passwordHint: "Minimum 5 characters. Any characters are allowed."
    },
    ru: {
      forgot: "Забыли пароль?",
      requestTitle: "Восстановление пароля",
      requestText: "Введите email, который использовали при регистрации в KisaMore. Мы отправим ссылку для установки нового пароля.",
      requestSubmit: "Отправить ссылку",
      requestSuccess: "Если аккаунт с таким email существует, ссылка для восстановления отправлена.",
      confirmTitle: "Новый пароль",
      confirmText: "Введите новый пароль для вашего аккаунта KisaMore.",
      confirmSubmit: "Сохранить пароль",
      confirmSuccess: "Пароль изменён. Теперь можно войти.",
      passwordHint: "Минимум 5 любых символов."
    },
    de: {
      forgot: "Passwort vergessen?", requestTitle: "Passwort zurücksetzen", requestText: "Geben Sie die E-Mail-Adresse Ihres KisaMore-Kontos ein.", requestSubmit: "Link senden", requestSuccess: "Wenn ein Konto existiert, wurde ein Link gesendet.", confirmTitle: "Neues Passwort", confirmText: "Geben Sie ein neues Passwort ein.", confirmSubmit: "Passwort speichern", confirmSuccess: "Passwort geändert. Sie können sich jetzt anmelden.", passwordHint: "Mindestens 5 beliebige Zeichen."
    },
    fr: {
      forgot: "Mot de passe oublié ?", requestTitle: "Réinitialiser le mot de passe", requestText: "Saisissez l’e-mail de votre compte KisaMore.", requestSubmit: "Envoyer le lien", requestSuccess: "Si le compte existe, un lien a été envoyé.", confirmTitle: "Nouveau mot de passe", confirmText: "Saisissez un nouveau mot de passe.", confirmSubmit: "Enregistrer", confirmSuccess: "Mot de passe modifié. Vous pouvez vous connecter.", passwordHint: "5 caractères minimum, quels qu’ils soient."
    },
    es: {
      forgot: "¿Olvidaste la contraseña?", requestTitle: "Restablecer contraseña", requestText: "Introduce el correo de tu cuenta KisaMore.", requestSubmit: "Enviar enlace", requestSuccess: "Si la cuenta existe, se ha enviado un enlace.", confirmTitle: "Nueva contraseña", confirmText: "Introduce una nueva contraseña.", confirmSubmit: "Guardar contraseña", confirmSuccess: "Contraseña cambiada. Ya puedes iniciar sesión.", passwordHint: "Mínimo 5 caracteres, de cualquier tipo."
    },
    it: {
      forgot: "Password dimenticata?", requestTitle: "Reimposta password", requestText: "Inserisci l’email del tuo account KisaMore.", requestSubmit: "Invia link", requestSuccess: "Se l’account esiste, è stato inviato un link.", confirmTitle: "Nuova password", confirmText: "Inserisci una nuova password.", confirmSubmit: "Salva password", confirmSuccess: "Password cambiata. Ora puoi accedere.", passwordHint: "Minimo 5 caratteri qualsiasi."
    },
    pt: {
      forgot: "Esqueceu a palavra-passe?", requestTitle: "Repor palavra-passe", requestText: "Introduza o email da sua conta KisaMore.", requestSubmit: "Enviar ligação", requestSuccess: "Se a conta existir, foi enviada uma ligação.", confirmTitle: "Nova palavra-passe", confirmText: "Introduza uma nova palavra-passe.", confirmSubmit: "Guardar", confirmSuccess: "Palavra-passe alterada. Já pode iniciar sessão.", passwordHint: "Mínimo de 5 caracteres quaisquer."
    },
    pl: {
      forgot: "Nie pamiętasz hasła?", requestTitle: "Reset hasła", requestText: "Wpisz e-mail konta KisaMore.", requestSubmit: "Wyślij link", requestSuccess: "Jeśli konto istnieje, link został wysłany.", confirmTitle: "Nowe hasło", confirmText: "Wpisz nowe hasło.", confirmSubmit: "Zapisz hasło", confirmSuccess: "Hasło zmienione. Możesz się zalogować.", passwordHint: "Minimum 5 dowolnych znaków."
    },
    zh: {
      forgot: "忘记密码？", requestTitle: "重置密码", requestText: "请输入 KisaMore 账户使用的邮箱。", requestSubmit: "发送重置链接", requestSuccess: "如果账户存在，重置链接已发送。", confirmTitle: "设置新密码", confirmText: "请输入新的账户密码。", confirmSubmit: "保存新密码", confirmSuccess: "密码已修改，现在可以登录。", passwordHint: "至少 5 个任意字符。"
    }
  }[language] || {
    forgot: "Forgot password?",
    requestTitle: "Reset password",
    requestText: "Enter the email used for your KisaMore account.",
    requestSubmit: "Send reset link",
    requestSuccess: "If this email belongs to an account, a reset link has been sent.",
    confirmTitle: "Set a new password",
    confirmText: "Enter a new password.",
    confirmSubmit: "Save new password",
    confirmSuccess: "Password changed. You can now sign in.",
    passwordHint: "Minimum 5 characters. Any characters are allowed."
  });
  ...
  function renderPasswordResetUi() {
    const labels = passwordResetUiText();
    $("#forgotPasswordButton").textContent = labels.forgot;
    $("#passwordResetRequestTitle").textContent = labels.requestTitle;
    $("#passwordResetRequestText").textContent = labels.requestText;
    $("#passwordResetRequestSubmit").textContent = labels.requestSubmit;
    $("#passwordResetConfirmTitle").textContent = labels.confirmTitle;
    $("#passwordResetConfirmText").textContent = labels.confirmText;
    $("#passwordResetConfirmSubmit").textContent = labels.confirmSubmit;
    $("#passwordResetPasswordHint").textContent = labels.passwordHint;
  }

  function openPasswordResetRequest() {
    const email = $("#authForm").elements.email.value.trim();
    if ($("#authDialog").open) $("#authDialog").close();
    const form = $("#passwordResetRequestForm");
    form.reset();
    form.elements.email.value = email;
    $(".form-error", form).classList.add("hidden");
    $(".form-success", form).classList.add("hidden");
    renderPasswordResetUi();
    $("#passwordResetRequestDialog").showModal();
  }

  function openPasswordResetConfirm(token) {
    const form = $("#passwordResetConfirmForm");
    form.reset();
    $("#passwordResetConfirmDialog").dataset.token = token;
    $(".form-error", form).classList.add("hidden");
    $(".form-success", form).classList.add("hidden");
    renderPasswordResetUi();
    $("#passwordResetConfirmDialog").showModal();
  }
  ...
  $("#passwordResetRequestForm").addEventListener("submit", async (event) => {
    ...
    const successNode = $(".form-success", form);
    ...
    try {
      ...
      successNode.textContent = passwordResetUiText().requestSuccess;
      successNode.classList.remove("hidden");
    } catch (error) {
      errorNode.textContent = error.message || t("actionError");
  ...
  $("#passwordResetConfirmForm").addEventListener("submit", async (event) => {
    event.preventDefault();
    const form = event.currentTarget;
    const token = $("#passwordResetConfirmDialog").dataset.token || "";
    const errorNode = $(".form-error", form);
    const successNode = $(".form-success", form);
    errorNode.classList.add("hidden");
    successNode.classList.add("hidden");
    try {
      ...
      successNode.textContent = passwordResetUiText().confirmSuccess;
      successNode.classList.remove("hidden");
      window.history.replaceState({}, document.title, window.location.pathname + window.location.hash);
      window.setTimeout(() => {
        $("#passwordResetConfirmDialog").close();
        openAuth();
      }, 900);
    } catch (error) {
      errorNode.textContent = error.message || t("actionError");
  ...
  if (resetToken) openPasswordResetConfirm(resetToken);

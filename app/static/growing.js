(() => {
  ...
  function showError(message) { showNotice(message, false); }
  function clearError() { notice.classList.add("hidden"); notice.classList.remove("success"); }
  ...
  $("#syncPlantsButton").addEventListener("click", async (event) => {
    const button = event.currentTarget;
    button.disabled = true;
    button.textContent = t("syncingPlants");
    clearError();
    try {
      await api("/api/growing/sync", { method: "POST" });
      showNotice(t("syncPlantsSuccess"), true);
    } catch (error) {
      console.error(error);
      showError(t("syncPlantsError"));
    } finally {
